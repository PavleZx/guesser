    ...
    cleaned_messages.append({
        ...
        "content": [
            {"type": "text", "text": f"Here is the current screen with a 50x50 grid overlay. "
                                      ...
                                      f"You can perform different types of actions: "
                                      f"- 'click' for regular left clicks "
                                      f"- 'doubleclick' for double-clicking "
                                      f"- 'rightclick' for right-clicking "
                                      f"- 'type' for typing text (you must specify what to type) "
                                      f"Please respond with the action and coordinates in one of these EXACT formats: "
                                      ...
                                      f"Examples: "
                                      f"click, column=25, row=30, \"Submit button\" "
                                      f"doubleclick, column=10, row=15, \"File icon\" "
                                      f"rightclick, column=35, row=20, \"Menu area\" "
                                      f"type > \"Hello World\", column=40, row=25, \"Text input field\""},
...
def get_guess_again(client, action_info, messages):
    ...
    cleaned_messages.append({
        "role": "user",
        "content": [
            {"type": "text", "text": f"Here is the screenshot with a 50x50 grid overlay. "
                                     f"The red square marks your previous choice: {action_info['summary']} "
                                     f"Please verify if the red square is correctly positioned for this action. "
                                     f"If correct, respond with the SAME action and coordinates. "
                                     f"If incorrect, provide the corrected action and coordinates. "
                                     f"Use the same format as before: "
                                     f"For click/doubleclick/rightclick: <action>, column=<number>, row=<number> "
                                     f"For typing: type > \"<text to type>\", column=<number>, row=<number>"},
            {"type": "image_url", "image_url": {"url": f"data:image/png;base64,{base64_image}"}}
        ]
    })
...
def tell_gpt_the_goal(goal, messages):
    ...
    messages.append({
        "role": "user",
        "content": f"You will help achieve the following goal: '{goal}'. "
                   f"You can perform these actions on my screen: "
                   f"- 'click' for regular left mouse clicks "
                   f"- 'doubleclick' for double-clicking (useful for opening files/apps) "
                   f"- 'rightclick' for right-clicking (to open context menus) "
                   f"- 'type' for typing text into input fields "
                   f"Each action requires you to specify grid coordinates (column, row). "
                   f"For typing, you must also specify the exact text to type. "
                   f"After each action, you'll see an updated screenshot with your choice highlighted "
                   f"in different colors: Red=click, Orange=doubleclick, Magenta=rightclick, Cyan=type. "
                   f"You can verify and correct your choices before the action is executed."
    }) 
    response = client.chat.completions.create(
        model="gpt-4o-mini",
        messages=messages
