...
with mp_pose.Pose() as pose:
    ...
    while cap.isOpened():
        ...
        if results.pose_landmarks:
            ...
            ellipse_center = (int(ellipse_center[0] + 100 * scaling_factor), int(ellipse_center[1] - 100 * scaling_factor))
            
            cv2.ellipse(
                frame,
                ellipse_center,
                (int(ellipse_width // 6), int(ellipse_height // 6)),
                0,  # rotation angle
                0,  # start angle (full ellipse)
                360,  # end angle (full ellipse)
                (0, 0, 0),  # black color for outline
            ...
            ellipse_center = (int(ellipse_center[0] - 185 * scaling_factor), int(ellipse_center[1] - 20 * scaling_factor))
            cv2.ellipse(
                frame,
                ellipse_center,
                (int(ellipse_width // 6), int(ellipse_height // 6)),
