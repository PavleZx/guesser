    ...
    cleaned_messages.append({
        ...
        "content": [
            {"type": "text", "text": f"Here is the current screen with a 50x50 grid overlay. "
                                      ...
                                      f"You may need to do some other actions first to reach the goal. As you would do when using a computer. "
                                      f"Please do THREE things in your response: "
                                      ...
                                      f"3. Choose an action for this step: click, double-click, right-click, or type."
                                      f"Respond STRICTLY in this format: action: <action>, column=<number>, row=<number>, \"<element description>\""
                                      f"choose an action that is ecessary to be performed on the element, and if the action is type provide the text to be typed like this: action: type(\"text to be typed\")..."},
        ...
        "content": [
            {"type": "text", "text": f"Here is the screenshot of my screen with a 50x50 grid overlay. "
                                     f"Assume that the red square was placed most likely correctly the first time. "
                                      ...
                                      f"Please check carefully if the red square is on the correct place. If you see that it is 3 squares up and 3 squares left from the required element, then add 3 to the row and 3 to the column. "
                                      f"If it is correct, you do not need to change the chosen column and row, so respond strictly with this format: column=<number>, row=<number>. "
...
def highlight_square(reply, filename="shot.png", n_lines=50):
    ...
    try:
        ...
        col = int((parts[0] if "=" in parts[0] else parts[1]).split("=")[1])
        row = int((parts[1] if "=" in parts[1] else parts[2]).split("=")[1])
        ...
        img = cv2.addWeighted(overlay, 1.0, img, 0.7, 0)
...
def parse_element_from_reply(reply):
    ...
    reply = "".join(reply.split(",")[1:]).strip()  # Get everything after the first comma
    ...
    try:
        ...
        if '"' in reply:
            ...
            parts = reply.split('"')
            if len(parts) >= 2:
                return action, parts[-2]  # Get the content of the last quote
        
        # Fallback: take everything after the second comma
        parts = reply.split(',')
        if len(parts) >= 3:
            return action, ','.join(parts[2:]).strip()
...
def tell_gpt_the_goal(goal, messages):
    ...
    messages.append({
        ...
        "content": f"You will help achieve the following goal by clicking or typing on my screen: '{goal}'. "
                   f"To achieve the goal, which may be to (click on something or type something, or open something), you may have to "
                   f"click or close some other things first. You will be allowed a several actions to do on my pc, but first chose an action to be performed. Details explained later."
                   f"Each click or action will be determined by you. After each action, you will be shown "
                   f"an updated screenshot with a grid and your chosen square highlighted. THE ACTION WILL HAPPEN AT THE RED SQUARE THAT YOU CHOSE. "
                   f"You must confirm or correct your choice of action location on the screen if necessary."
    }) 
    response = client.chat.completions.create(
        model="gpt-4o-mini",
        messages=messages
...
def main():
    ...
    for i in range(10):
        ...
        # probaj napravit da se koriste zadnjih 10 message-a ili 8, ali uvijek da je prvi user message o goal-u
