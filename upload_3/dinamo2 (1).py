...
def draw_person(frame, landmarks, line_color, fill_color, w, h):
    """Draw pose effect for one person"""
...
def separate_poses(all_landmarks, w, h):
    """Separate detected poses into left and right person based on x-coordinate"""
    if len(all_landmarks) <= 1:
        return all_landmarks
    
    # Calculate center x-coordinate for each pose
    pose_centers = []
    for landmarks in all_landmarks:
        ...
        pose_centers.append((center_x, landmarks))
    
    # Sort by x-coordinate and return up to 2 poses
    pose_centers.sort(key=lambda x: x[0])
    return [pose[1] for pose in pose_centers[:2]]

# Initialize pose detection for multiple people
with mp_pose.Pose(
    ...
    while cap.isOpened():
        ...
        # Process frame multiple times to detect multiple people
        rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)

        # frame[:] = (0, 255, 0)
        
        # Split frame into sections to help detect multiple people
        detected_poses = []
        
        # Try to detect poses in different regions
        regions = [
            (0, 0, w//2, h),      # Left half
        ...
        for region in regions:
            x1, y1, x2, y2 = region
            region_rgb = rgb[y1:y2, x1:x2]
            results = pose.process(region_rgb)
            ...
            if results.pose_landmarks:
                ...
                for landmark in results.pose_landmarks.landmark:
                    ...
                    new_landmark.x = (landmark.x * (x2 - x1) + x1) / w
                    new_landmark.y = (landmark.y * (y2 - y1) + y1) / h
                    new_landmark.z = landmark.z
                    new_landmark.visibility = landmark.visibility
                
                # Check if this pose is significantly different from already detected poses
                is_duplicate = False
                for existing_pose in detected_poses:
                    # Compare nose positions to avoid duplicates
                    existing_nose = existing_pose.landmark[mp_pose.PoseLandmark.NOSE]
                    new_nose = adjusted_landmarks.landmark[mp_pose.PoseLandmark.NOSE]
                    
                    distance = np.sqrt((existing_nose.x - new_nose.x)**2 + (existing_nose.y - new_nose.y)**2)
                    if distance < 0.1:  # If poses are too close, consider it duplicate
                        is_duplicate = True
                        break
        ...
        # Limit to 2 people and separate them
        if detected_poses:
            separated_poses = separate_poses(detected_poses, w, h)
            
            # Draw each detected person
            for i, pose_landmarks in enumerate(separated_poses[:2]):
                # You can use different colors for different people if desired
                person_line_color = line_color
                person_fill_color = fill_color
                
                # Optional: Different colors for each person
                if len(separated_poses) > 1:
                    if i == 0:  # First person
                        if current_color_mode == 'rgb':
                            person_line_color = (b, g, r)
                            person_fill_color = (b, g, r)
                    else:  # Second person - slightly different color
                        if current_color_mode == 'rgb':
                            person_line_color = (r, b, g)
                            person_fill_color = (r, b, g)
                
                draw_person(frame, pose_landmarks.landmark, person_line_color, person_fill_color, w, h)
